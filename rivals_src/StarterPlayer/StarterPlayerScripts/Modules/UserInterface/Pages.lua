...
function v_u_17.OpenPickWeaponsPage(p22)
    p22.PageSystem:OpenPage(v_u_8:Get("Settings Pick Weapons List") == "List" and "PickWeaponsList" or "PickWeapons", true)
end
...
function v_u_17._CheckInterruption(p24)
    if p24.PageSystem.CurrentPage then
        if v_u_14:IsVisible() and p24.PageSystem.CurrentPage.Name ~= "PickEmote" or (v_u_13.Enabled or v_u_12.EditorEnabled) then
    ...
    if p25.PageSystem.CurrentPage then
        ...
        if not v_u_11:GetDuel(v_u_4.LocalPlayer) then
            ...
            if p25._welcomeback_popped_up or not v_u_8:Get("WelcomeBackGiftReady") then
                if p25._patchnotes_popped_up or (v_u_8:Get("LastPatchNotesVersion") == v_u_5.GAME_VERSION or v26 <= 3) then
                    if p25._mobileduels_popped_up or (v_u_5.IS_MOBILE_SERVER or (v26 <= 0 or (v26 > 3 or v_u_9.CurrentControls ~= "Touch"))) then
                        if v26 > 10 and v_u_5.IS_HUB_SERVER and (v_u_7:IsAprilFools() and v_u_8:Get("AprilFoolsDialogAcknowledged") == -1 or not v_u_7:IsAprilFools() and v_u_8:Get("AprilFoolsDialogAcknowledged") ~= -1) then
...
function v_u_17._Setup(_)
    task.defer(function()
        if not v_u_6:HasBasicAccess(v_u_4.LocalPlayer.UserId, v_u_8:Get("GroupRank")) then
            ...
            v_u_4.LocalPlayer.PlayerScripts.UserInterface:WaitForChild("Prompts"):WaitForChild("SendOfflineGiftRewards"):Destroy()
        end
